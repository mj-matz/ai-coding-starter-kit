...
export interface RunDetailDrawerProps {
  run: Mt5TesterRun | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onUseSettings: (fullRun: Mt5TesterRun) => void;
}

export function RunDetailDrawer({ run, open, onOpenChange, onUseSettings }: RunDetailDrawerProps) {
  // Track which run's data has been loaded to avoid refetching on re-render.
  ...
  // Derived loading state — no synchronous setState reset needed.
  const isLoading = open && run != null && fetchedRunId !== run.id && !fetchError;

  // Fetch full run detail + trades. All setState calls are inside async callbacks.
  useEffect(() => {
    if (!open || !run) return;
  ...
  }, [open, run, fetchedRunId]);

  // Show partial data from the list while the full detail is loading.
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="w-full overflow-y-auto border-white/10 bg-[#0d0f14] p-0 text-white sm:max-w-2xl"
      >
        {/* Header */}
          ...
          {displayRun && (
            <p className="mt-1 text-xs text-slate-400">
              ...
              {displayRun.finished_at &&
                ` · Finished ${formatDate(displayRun.finished_at)}`}
            </p>
          )}
        </SheetHeader>
        ...
        <div className="space-y-6 px-6 py-5">
          ...
          <section>
            ...
            <div className="rounded-xl border border-white/10 bg-black/20 px-4 py-1">
              ...
              <MetricRow
                label="From"
                value={displayRun?.from_date?.slice(0, 10) ?? "—"}
              />
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-white/10 hover:bg-transparent">
                      ...
                      <TableHead className="text-right text-slate-400">Vol</TableHead>
